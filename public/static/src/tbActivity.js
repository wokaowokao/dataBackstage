var table = 'tbActivity';
var url = '/index.php/Index/getData';
var info = _info.tbActivity;
var colums = [_colums.tbActivity];
//是否复杂表头标示 用来区别不同的绑定事件
var isSecialExcelTitle = 0;
...
$(function() {
    ...
    $('#o5').click(function() {
        ...
        var foo = [];
        $("[name='searchDuanType[]']:checked").each(function() {
            foo.push($(this).val());
        })
        if (foo.length) searchDuanType = foo.join(',');
        else searchDuanType = '';
        /*$("[name='searchDuanType']:checked").each(function(index, element) {
            searchDuanType += $(this).val() + ",";
        });

     = $('input[name="searchDuanType[]"]').val();*/

        if ($('#from').datebox('getValue') || $('#to').datebox('getValue') || searchStr || searchId) {
            dateType = 5;
            clickDate($(this));
        } else {
            alert('请选择至少一个条件！');
    ...
    $('#download').click(function() {
            var str = '';
            if (searchStr) str += '/searchStr/' + base64encode(encodeURIComponent(searchStr));
            if (searchId) str += '/searchId/' + searchId;
            if (searchDuanType) str += '/searchDuanType/' + searchDuanType;
            var url = '/index.php/ExcelExport/excute/table/' + table + '/dateType/' + dateType + /title/ + _title + (str ? str : '');
            if (dateType == 5) {
...
function checkAll(obj) {
    $(".h22 input[type='checkbox']").prop('checked', $(obj).prop('checked'));
}
