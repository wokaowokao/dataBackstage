var table = 'baiduVisit';
var url = '/index.php/Index/getData';
var info = _info.baiduVisit;
var colums = [_colums.baiduVisit];
...
$(function() {
    ...
    $('#o5').click(function() {
        searchFoo = $('#searchId').val();
        if (searchFoo) {
            searchFoo = searchFoo.split(',');
            searchFoo.forEach(function(k, v) {
                searchFoo[v] = "'" + k + "'";
            })
            searchId = searchFoo.join(',');
        }else{
            searchId = '';
        }
        ...
        if ($('#from').datebox('getValue') || $('#to').datebox('getValue') || searchId) {
            dateType = 5;
            clickDate($(this));
        } else {
            alert('请选择至少一个条件！');
